...
function arraySortInfo(inputArray){
    let result=inputArray.every((num)=>Number.isInteger(num))
    if(!result){
        return "Some element is not number"
    }

    if(inputArray.every((element,index)=> index>0 ? element >= inputArray[index-1] : true)){
        return "Array is sorted in ascending"
    }else if(inputArray.every((element,index)=> index>0 ? element <= inputArray[index-1] : true)){
        return "Array is sorted in descending"
    }else{
        return "Array is not sorted"
    }

    /*let sortArrayAscend=[...inputArray].sort((a, b) => a - b)
    let sortArrayDescend=[...inputArray].sort((a, b) => b - a)
    if(JSON.stringify(sortArrayAscend) === JSON.stringify(inputArray)){
        return "Array is sorted in ascending"
    }else if(JSON.stringify(sortArrayDescend) === JSON.stringify(inputArray)){
        return "Array is sorted in descending"
    }else{
        return "Array is not sorted"
    }
